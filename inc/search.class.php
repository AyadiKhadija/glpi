...
class Search {
   ...
   **/
   static function displayData(array &$data) {
      ...
      //Special case for displaying default user image when none is specified
      if ($data['itemtype'] == "User") {
         $picID = array_search('Picture', array_column($data['data']['cols'], 'name'));
         if ($picID !== false) {
            foreach ($data['data']['rows'] as $key => $val) {
               if (!isset($data['data']['rows'][$key]['raw']["ITEM_$picID"])) {
                  $data['data']['rows'][$key]['raw']["ITEM_$picID"] = "";
                  $data['data']['rows'][$key][$picID]['displayname'] = User::getSpecificValueToDisplay('picture', "", ['html' => true]);
               }
            }
         }
      }
