...
class Search {
   ...
   **/
   static function showMap($itemtype, $params) {
      ...
      $data = self::getDatas($itemtype, $params);
      $data['params'] = $params_encoded;
      $data['itemtype_name'] = class_exists($itemtype) ? $itemtype::getTypeName($data['data']['totalcount']) :
      ...
      $data['full_target'] = $target . (str_contains($target, '?') ? '&' : '?') . $parameters;
