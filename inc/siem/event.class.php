...
class Event extends \CommonDBTM {
   ...
   public static function getAdditionalMenuContent()
   {
      ...
      $menu['plugin_siem_event']['options']['PluginSiemHost']['title'] = Host::getTypeName(2);
      $menu['plugin_siem_event']['options']['PluginSiemHost']['page'] = Host::getSearchURL(false);
      $menu['plugin_siem_event']['options']['PluginSiemHost']['links']['search'] = Host::getSearchURL(false);
      $menu['plugin_siem_event']['options']['PluginSiemHost']['links']['add'] = Host::getFormURL(false);
      $menu['plugin_siem_event']['options']['PluginSiemService']['title'] = Service::getTypeName(2);
      $menu['plugin_siem_event']['options']['PluginSiemService']['page'] = Service::getSearchURL(false);
      $menu['plugin_siem_event']['options']['PluginSiemService']['links']['search'] = Service::getSearchURL(false);
      $menu['plugin_siem_event']['options']['PluginSiemService']['links']['add'] = Service::getFormURL(false);
      return $menu;
   }
...
//         ] + $input);
   ...
   public static function getActiveAlerts()
   {
      global $DB;
      $eventtable = self::getTable();
      $servicetable = Service::getTable();
      $iterator = $DB->request([
         'SELECT' => [
            'glpi_siemevents.*',
            $servicetable.'.name AS service_name',
            $servicetable.'.is_stateless AS service_stateless'
         ],
         'FROM' => $eventtable,
         'LEFT JOIN' => [
            $servicetable => [
