...
class Calendar extends CommonDropdown {
   ...
   **/
   function isHoliday($date) {
      ...
      $iterator = $DB->request('glpi_calendars_holidays', [
         'COUNT' => 'cpt',
         'INNER JOIN' => [
            'glpi_holidays' => [
               'FKEY' => [
         ...
         'WHERE' => [
            ...
            'OR' => [
               'AND' => [
                  $date => ['<=', 'glpi_holidays.end_date'],
                  $date => ['>=', 'glpi_holidays.begin_date']
               ],
               'AND' => [
                  'glpi_holidays.is_perpetual' => 1,
                  new QueryExpression("MONTH(`end_date`)*100 + DAY(`end_date`)") =>
                     ['>=', date('nd', strtotime($date))],
                  new QueryExpression("MONTH(`begin_date`)*100 + DAY(`begin_date`)") =>
                     ['>=', date('nd', strtotime($date))]
               ]
            ]
         ]
