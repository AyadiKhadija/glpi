...
class WebhookTrigger extends CommonDBChild {
   ...
   static function getTriggerOptions() {
      ...
      foreach ($CFG_GLPI['globalsearch_types'] as $searchopt) {
         $triggers[$searchopt]['add'] = __('Add');
         $triggers[$searchopt]['update'] = __('Update');
         $triggers[$searchopt]['create'] = __('Create');
         $triggers[$searchopt]['purge'] = __('Purge');
         $item = new $searchopt();
         $item->getEmpty();
         if ($item->maybeDeleted()) {
            $triggers[$searchopt]['delete'] = __('Delete');
            $triggers[$searchopt]['restore'] = __('Restore');
         }
      }
   ...
   function showForm($ID, array $options = []) {
      ...
      echo "<td><label for='itemtype'>" . __('Item type') . "</label></td><td>";
      ...
      Ajax::updateItemOnSelectEvent("dropdown_itemtype$rand", "trigger_action",
      ...
      echo Html::scriptBlock("$('#dropdown_itemtype$rand').change()");
      echo "</td>";
   ...
   static function showForWebhook($item) {
      ...
      if (count($iterator)) {
         ...
         foreach($iterator as $data) {
            ...
            echo "</td>";
            echo "<td>".$data['itemtype']::getTypeName(1)."</td>";
